...
class VerifyCert extends React.Component {
    // ? function is called when the fetch button is clicked
    onVerify = (formValues) => {
        console.log(formValues);
        if (formValues) {
            // TODO : update the action creator here
            this.props.verifyCert(formValues);
            ...
            <div class="flex items-center h-full w-full bg-teal-lighter">
                <div class="w-full bg-transparent rounded shadow-md p-8 m-4 md:max-w-sm md:mx-auto">
                    <form
                        ...
                        onSubmit={this.props.handleSubmit(this.onVerify)}
                    >
                        <Field
                            label="UID Number"
                            name="uid_number"
                            type="text"
                            placeholder="eg: 0f33076c259d8813da69"
        ...
        return (
            <div className="my-10 pt-20">
                <h1 class="block w-full text-center text-grey-darkest mb-6 text-4xl tracking-tight">
                    ...
                    <span className="text-orange-500 uppercase">Verify</span>{" "}
                    your certificate .
                </h1>
...
const validate = (formValues) => {
    const errors = {};
    if (!formValues.uid_number) {
        errors.uid_number = "UID Number required";
    }
    return errors;
};
...
//     return {
//         verifyCertDetails: state.cert,
//     };
// };

export default connect(null, { verifyCert })(
    reduxForm({
        form: "VerifyCert",
        validate,
    })(VerifyCert)
