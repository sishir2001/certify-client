...
import { add_event, null_events_message } from "../../actions/";
// import { Link } from "react-router-dom";

class AddEvents extends React.Component {
    componentDidMount() {
        this.props.null_events_message();
    }
    // ? function is called when the fetch button is clicked
    onAddEvent = (formValues) => {
        ...
        return (
            <div className="md:max-w-sm md:mx-auto mt-4 ">
                <form
                    ...
                    onSubmit={this.props.handleSubmit(this.onAddEvent)}
                >
                    <Field
                        label="Event Name"
                        name="event_name"
                    ...
                    <Field
                        ...
                        name="certificate_default_content"
                        type="text"
                        placeholder="For ...."
                        component={this.renderInput}
                    />
    ...
    render() {
        console.log(this.props.response_message);
...
// export default connect(null)(AddEvents);
...
const validate = (formValues) => {
    ...
    if (!formValues.event_code) {
        errors.event_code = "Event Code Required";
    }
    if (!formValues.certificate_default_content) {
        errors.certificate_default_content = "Event Description Required";
    }
    if (!formValues.template_number) {
        errors.template_number = "Template Number Required";
    }
    return errors;
};
...
const mapStateToProps = ({ cert, auth }) => {
    return {
        response_message: cert.added_event_message,
        username: auth.userName,
        password: auth.password,
    };
};

export default connect(mapStateToProps, { add_event, null_events_message })(
    reduxForm({
        form: "AddEvents",
        validate,
    })(AddEvents)
